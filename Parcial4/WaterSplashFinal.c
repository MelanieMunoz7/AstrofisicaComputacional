...
int nFluid, nFluid2, nPart; //Numero de particulas de fluido y numero de particulas de la simulacion 8las de fluido y las de frontera)
...
void ics(int nx, int ny, double dx_fluid, double dy_fluid, double dx_border,double dy_border, double Lx, double Ly,double t);
...
int main(int argc, char *argv[])
{
  ...
  double dt = 3.125e-6; //3.125e-6 El dt cambiara en funcion de la longitud de suavizado h (dx_fluid) al cuadrado y la viscosidad cinetica de las particulas (1e-6 m²/s²), multiplicadas por 0.125
  ...
  TraslacionEnX= 1.75e-4; 
  TraslacionEnY = 4e-4;

  //Parametros de lanzamiento
  maximaVelocidadInicial = 0.02; // Si c = 10Vmax, entonces Vmax = c/10 m/s
  anguloLanzamiento = M_PI/6; //30°


  //Cambiando los dx y dy para las particulas de fluido
  dx_fluid = diametroCirculo/nx;
  dy_fluid = diametroCirculo/ny;

  //Sin cambio para las particulas del borde
  dx_border = Lx/nx;
  dy_border = Ly/nx;

  nFluid = nx*ny; //Cantidad de Particulas del fluido 
 ...
 //El tiempo que avanza la simulacion      
  ...
  ics(nx, ny, dx_fluid, dy_fluid,dx_border,dy_border, Lx, Ly,t);
...
void ics(int nx, int ny, double dx_fluid, double dy_fluid, double dx_border,double dy_border, double Lx, double Ly, double t)
{
  ...
  //Variables donde almacenaremos las posciones de las particulas del fluido
  //Y ver si estan dentro del circulo
  double posX,posY;
  ...
  CentroCirculoInicialX = 2.75e-4;
  CentroCirculoInicialY = 5e-4;
  ...
  for( j=0; j<ny; j++)
    {
      for( i=0; i<nx; i++)
          ...
          posX = i*dx_fluid+dx_fluid/2.0 + TraslacionEnX;
          posY = j*dy_fluid+dy_fluid/2.0 + TraslacionEnY;

          //Calculando la distanica al centro del circulo
          distanciaEnXCentro = posX - centroCirculoEnX;
          distanciaEnYCentro = posY - centroCirculoEnY; 

          distanciaCentroTotal = distanciaEnXCentro * distanciaEnXCentro + 
          distanciaEnYCentro * distanciaEnYCentro; 

          if (distanciaCentroTotal<= radioCirculo*radioCirculo)
          {
	          part[counter].id = counter;
	          part[counter].pos[X] = posX;
	          part[counter].pos[Y] = posY;
  ...
  nPart = counter; //Tiene que ser igual al contador, porque el valor cambia dependiendo de que particulas hay el circulo
  ...
  for( i=0; i<=npV; i++)
    {
      ...
      part[counter].dWx = NULL;
      part[counter].dWy = NULL;
      part[counter].type = -1; //Particula virtual
      counter++;
    }
  ...
  fclose(frBorder);
                                                                    HASTA AQUÍ EL BORDE DE LA DERECHA
...
/*Agregando una frontera extra para logar la interaccion que se 
obtienen con las particulas de tipo 2*/

//Borde Inferior extra 
  nPart = counter;
  ...
  for( i=0; i<=npV; i++)
    {
      part[counter].id = counter;
      part[counter].pos[X] = i*dx_border/2.0;
      part[counter].pos[Y] = 0.0; //Para que este por debajo del borde actual 
      part[counter].vel[X] = 0.0;
      //part[counter].vel[X] = -vBoundary;
      part[counter].vel[Y] = 0.0;
      part[counter].accel[X] = 0.0;
...
{
  int i;
  double primeraParte,segundaParte;
  int rhoInicial = 1000; 
  int gamma = 7;

  for( i=0; i<nPart; i++ )
    {
      part[i].c = 0.01; // c = 10 * velocidad_max
      primeraParte = part[i].c * part[i].c * rhoInicial * (1/gamma);
      segundaParte = part[i].rho / rhoInicial;

      part[i].p = primeraParte * (pow(segundaParte,gamma)-1);
    }
}
