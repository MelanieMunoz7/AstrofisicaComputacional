...
int acceleration(void)
{
  ...
  int i, j;
  double dr[3]; 
  double r, R, r3; //Radio, radio esferico, radio al cubo
  double eps2; //Longitud de suvizado 

  eps2 = eps*eps;

  double m = 1.0; // masa para partículas de prueba
  double Mh, rh; //Masa halo materia oscura, y el radio 
  double Mb, rb; //Masa del bulbo y el radio 
  double y, Sigma0, Md, Rd, lambda0, z0, I0, I1, K0, K1, nu; //Md: masa del disco, el resto tambien son parametros del disco
  double axh, axd, axb;
  double ayh, ayd, ayb;
  double azh, azd, azb;
  ...
  Md = 5.59;
  Rd = 3.4;
  Sigma0 = Md/(2*M_PI*Rd*Rd); //Densidad central del disco
  lambda0 = 0.0001;
  z0 = 0.2;
  nu = 75.0;
