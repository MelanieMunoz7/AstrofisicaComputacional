...
int main(void)
{
  ...
  int indices_ordenados[nPuntos][nVecinos]; //numero de filas es nPuntos, nuemro de columnas es nVecinos
  //Cambios que se van haciendo que se almacenan
...
int calcula_distancia(const int nPuntos, double x[], double y[], double z[], const int nVecinos, int indices_ordenados[][nVecinos])
{
  //Aca si se trabaja con toda la matriz
  int i, j, indices[nPuntos];
  double distancia[nPuntos];
  
  // calcula la distancia a cada punto
  //En particular esto trastala el sistema de refencia para que se le calcule las diferentes distancias
  //La distanica de un punto particular al origen
  //La distancia de ese punto a sus vecinos, note que esto implica trasladar el sistema de referencia al punto que se le desean calcular los vecinos
  for( i=0; i<nPuntos; i++ )
    {
      for( j=0; j<nPuntos; j++ )
	{
      ...
      //Porque note que el ciclo debio recorrer cada punto, por ende el valor de puntoConVecinos debe ser igual a i
      ...
      if( i==puntoConVecinos )
	{
	  ...
	  for( j=0; j<nVecinos; j++ )
	    printf("%d ",indices_ordenados[puntoConVecinos][j]); //Entonces solo imprimo la componente de la matriz que quiere el usuario
	  printf("\n");
	}
...
{
 //Aca ya solo se trabaja con el la fija que el usuario quiere
